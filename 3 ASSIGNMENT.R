...
# a) Select CustomerName and Product
sales_data %>% select(CustomerName, Product)

# b) Select numeric columns
sales_data %>% select(where(is.numeric))

# c) Remove Discount column
sales_data %>% select(-Discount)

# d) Filter Pune customers
sales_data %>% filter(City == "Pune")

# e) Filter multiple conditions (Price > 20000 and Quantity > 5)
sales_data %>% filter(Price > 20000 & Quantity > 5)
...
# a) Total revenue by city
sales_data %>% group_by(City) %>% summarise(TotalRevenue = sum(Revenue))

# b) Average price by product
sales_data %>% group_by(Product) %>% summarise(AveragePrice = mean(Price))

# c) Total quantity sold by product
sales_data %>% group_by(Product) %>% summarise(TotalQuantity = sum(Quantity))

# d) Maximum revenue by city
sales_data %>% group_by(City) %>% summarise(MaxRevenue = max(Revenue))

# e) Minimum revenue by product
sales_data %>% group_by(Product) %>% summarise(MinRevenue = min(Revenue))
...
# 1. Count orders per customer
sales_data %>% group_by(CustomerName) %>% summarise(OrderCount = n())

# 2. Total revenue per customer
sales_data %>% group_by(CustomerName) %>% summarise(TotalRevenue = sum(Revenue))

# 3. Average order value per customer
sales_data %>% group_by(CustomerName) %>% summarise(AverageOrderValue = mean(Revenue))

# 4. Top customer (highest revenue)
sales_data %>% group_by(CustomerName) %>% summarise(TotalRevenue = sum(Revenue)) %>%
  arrange(desc(TotalRevenue)) %>% head(1)
